...
class ClassifierOps(object):
    def __init__(self, loss, output, extra_loss=None):
        ...
        # Inputs, train op (used for ae_data augmentation).
        self.x = None
        self.label = None
        self.train_op = None
...
def single_layer_classifier(h, l, nclass, scope='single_layer_classifier', smoothing=None):
    with tf.compat.v1.variable_scope(scope):
        h0 = tf.compat.v1.layers.flatten(h)
        logits = tf.compat.v1.layers.dense(h0, nclass)
        output = tf.argmax(logits, 1)
        if smoothing:
            l -= abs(smoothing) * (l - 1. / nclass)
        loss = tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=l)
