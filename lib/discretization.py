...
class DiscreteBottleneck(object):
    def __init__(self, hparams):
        ...
        self.hparams.z_size_per_residual = self.hparams.z_size // self.hparams.num_residuals
        ...
        self.hparams.means = tf.compat.v1.get_variable(
            name="means",
            shape=[
        ...
        if self.hparams.ema:
            ...
            with tf.compat.v1.colocate_with(self.hparams.means):
                self.ema_means = []
                for i in range(hparams.num_residuals):
    ...
    def nearest_neighbor(self, x, means):
        ...
        Args:
            x: Batch of encoder continuous latent states sliced/projected into shape [-1, num_blocks, block_dim].
        ...
        if self.hparams.soft_em:
            nearest_idx = tf.stack(
                [
                    tf.compat.v1.multinomial(-dist[:, i, :],
    ...
    def bit_to_int(self, x_bit, num_bits, base=2):
        ...
        """
        x_l = tf.stop_gradient(tf.compat.v1.to_int32(tf.reshape(x_bit, [-1, num_bits])))
        x_labels = []
        for i in range(num_bits):
            x_labels.append(x_l[:, i] * tf.compat.v1.to_int32(base) ** tf.compat.v1.to_int32(i))
        res = sum(x_labels)
        return tf.compat.v1.to_int32(res)
    ...
    def int_to_bit(self, x_int, num_bits, base=2):
        ...
        """
        x_l = tf.compat.v1.to_int32(tf.expand_dims(x_int, axis=-1))
        x_labels = []
        for i in range(num_bits):
            x_labels.append(
                tf.compat.v1.floormod(
                    tf.compat.v1.floordiv(tf.compat.v1.to_int32(x_l),
                                tf.compat.v1.to_int32(base) ** i), tf.compat.v1.to_int32(base)))
        res = tf.concat(x_labels, axis=-1)
        return tf.compat.v1.to_float(res)
    ...
    def embed(self, x, scope='embed'):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            c = tf.compat.v1.to_int32(tf.reshape(c, shape=new_shape))
            h1_shape = shape_x
            ...
            return tf.compat.v1.layers.dense(tf.nn.relu(h1), self.hparams.hidden_size,
                                   name="vcfin")
    ...
    def discrete_bottleneck(self, x, scope='bottleneck'):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            for i in range(self.hparams.num_residuals):
                ...
                if self.hparams.ema:
                    ...
                    with tf.control_dependencies([e_loss_res]):
                        update_means_res = tf.compat.v1.assign(self.hparams.means[i],
                                                     updated_ema_means_res)
            ...
            c = self.bit_to_int(tf.compat.v1.to_int32(x_means_bits),
                                num_bits=self.hparams.z_size,
                                base=2)

            x_means = tf.reshape(x_means, shape=tf.shape(x))
            h1 = x + tf.stop_gradient(x_means - x)
            h2 = tf.compat.v1.layers.dense(tf.nn.relu(h1), self.hparams.filter_size,
                                 name="vch2")
            res = tf.compat.v1.layers.dense(tf.nn.relu(h2), self.hparams.hidden_size,
                                  name="vcfin")
