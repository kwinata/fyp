...
class HeModifiedNormalInitializer(tf.initializers.random_normal):
    ...
    def __call__(self, shape, dtype=None, partition_info=None):
        ...
        std = tf.compat.v1.rsqrt((1. + self.slope**2) * tf.cast(tf.reduce_prod(shape[:-1]), tf.float32))
        return tf.compat.v1.random_normal(shape, stddev=std, dtype=dtype)


def encoder(x, scales, depth, latent, scope):
    activation = tf.nn.leaky_relu
    conv_op = functools.partial(
        tf.compat.v1.layers.conv2d, padding='same',
