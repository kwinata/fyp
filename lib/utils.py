...
class HookReport(tf.compat.v1.train.SessionRunHook):
    ...
    def before_run(self, run_context):
        del run_context
        fetches = tf.compat.v1.get_collection(self._REPORT_KEY)
        fetches = fetches + [tf.compat.v1.train.get_global_step()]
        return tf.compat.v1.train.SessionRunArgs(fetches)
    ...
    def after_run(self, run_context, run_values):
        ...
        if self.step // self._period != self._step_ratio:
            ...
            stop = time.time()
            tf.compat.v1.logging.info('kimg=%d  %s  [%.2f img/s]' %
    ...
    def log_tensor(cls, tensor, name):
        ...
        if cls._ENABLE:
            ...
            tf.compat.v1.add_to_collection(cls._REPORT_KEY, tensor)
...
def save_images(image, output_dir, cur_nimg, name=None):
    ...
    for name in names:
        with tf.compat.v1.gfile.Open(os.path.join(output_dir, name), 'wb') as f:
            f.write(image)
...
def to_png(x):
    ...
    with tf.Graph().as_default():
        with tf.compat.v1.Session() as sess_temp:
            x = tf.constant(x)
